...
/**
 * Unit tests for SendMosqueSuspendedNotification listener — edge cases.
 *
 * Requirements: 3.5, 3.6
 */
class SendMosqueSuspendedNotificationTest extends TestCase
{
    use RefreshDatabase;
    ...
    // -------------------------------------------------------------------------
    // Edge case: admin_user_id is null — Requirements: 3.5
    // -------------------------------------------------------------------------
    ...
    public function test_no_job_dispatched_when_admin_user_id_is_null(): void
    {
        ...
        $mosque                = Mosque::factory()->make(['status' => MosqueStatus::Suspended]);
        $mosque->admin_user_id = null;

        $event    = new MosqueSuspended($mosque, 1, Carbon::now());
        $listener = new SendMosqueSuspendedNotification();
    ...
    // -------------------------------------------------------------------------
    // Edge case: admin has no FCM tokens — Requirements: 3.6
    // -------------------------------------------------------------------------
